...
import com.ibm.api.psd2.api.common.MongoConnection;
import com.ibm.api.psd2.api.common.MongoDocumentParser;
...
public class SubscriptionDaoImpl implements SubscriptionDao
{
	...
	@Override
	public SubscriptionInfoBean getSubscriptionInfo(String username, String clientId, String accountId,
			String bankId) throws Exception
	{
		...
		FindIterable<Document> iterable = coll.find(
				and(eq("accountId", accountId), eq("bank_id", bankId), eq("username", username), eq("clientId", clientId)))
	...
	public List<SubscriptionInfoBean> getSubscriptionInfo(String username, String clientId, String bankId) throws Exception
	{
		...
		MongoCollection<Document> coll = conn.getDB().getCollection(subscriptions);
		FindIterable<Document> iterable = coll.find(
				and(eq("bank_id", bankId), eq("username", username), eq("clientId", clientId)))
				.projection(excludeId());
